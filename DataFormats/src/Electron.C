...
int  Electron::PassIDTight(TString ID) const{
  ...
  if(ID == "HNL_ULID_OLD_"     +Year)     return (PassID("HNL_ULID_OLDDef_FO_"+Year)    && PassID("HNL_ULID_Fake_"+Year));
  ...
  // HNL_ULID_OLDDef_FO* are OLD FO ie before changing Charge cut
  if(ID == "HNL_ULID_OLDDef_FO_"  +Year)   return (PassID("HNL_ULID_Conv_"+Year)   && PassID("HNL_ULID_CF_"+Year));
  // HNL_ULID_Def_FO_ is new default FO ID WIth medium charge cut
  ...
  if(ID == "HNL_ULID_OLD_FO_2018")      return PassID("HNL_ULID_OLD_FO_v9_a_"+Year);
  ...
  if(ID.Contains("HNL_ULID_OLD_FO_")){

    ///// V0 ID is BASIC loose ID with NO DEEPJET cuts                                                                                                                                                                                                     
    double BJetDeepJetCut = 999;
    if(ID.Contains("HNL_ULID_OLD_FO_v1")) BJetDeepJetCut = 0.025;  
    if(ID.Contains("HNL_ULID_OLD_FO_v2")) BJetDeepJetCut = 0.05; //// Version 2 == TOP 
    if(ID.Contains("HNL_ULID_OLD_FO_v3")) BJetDeepJetCut = 0.1;
    if(ID.Contains("HNL_ULID_OLD_FO_v4")) BJetDeepJetCut = 0.2;
    if(ID.Contains("HNL_ULID_OLD_FO_v5")) BJetDeepJetCut = 0.3;
    if(ID.Contains("HNL_ULID_OLD_FO_v6")) BJetDeepJetCut = 0.4;
    if(ID.Contains("HNL_ULID_OLD_FO_v7")) BJetDeepJetCut = 0.5;
    if(ID.Contains("HNL_ULID_OLD_FO_v8")) BJetDeepJetCut = 0.75;
    if(ID.Contains("HNL_ULID_OLD_FO_v9")) BJetDeepJetCut = 0.9;
    bool ApplyCloseJet_CvsBScore=false;
    bool ApplyCloseJet_CvsLScore=false;
    double CBJetDeepJetCut = -999;
    double CLJetDeepJetCut = 999;
    ...
    if(!PassID("HNL_ULID_OLDDef_FO_"+Year)) return false;
    if(!PassID("HNL_ULID_Fake_"+Year)){
      if(this->HasCloseJet()){
        if(CloseJet_Ptratio() < CloseJet_PtratioCut)         return false;
        if(this->CloseJet_BScore()    >  BJetDeepJetCut) return false;
