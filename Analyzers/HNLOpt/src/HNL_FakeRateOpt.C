...
ç
  ...
  if (ElectronOpt){
	...
	for(auto iCC : vCC){
	  for(auto iMVAB : vMVAB) OptElectronsIDs.push_back(iTrig+iConv+iCC+iMVAB+"MVAEC-0.8_ISOB0.15_ISOEC0.15_DXYB1EC1");
	  for(auto iMVAEC : vMVAEC) OptElectronsIDs.push_back(iTrig+iConv+iCC+iMVAEC+"MVAB-0.5_ISOB0.15_ISOEC0.15_DXYB1EC1");
	  OptElectronsIDs.push_back(iTrig+iConv+iCC+"POGT_ISOB0.15_ISOEC0.15_DXYB1EC1");
	  OptElectronsIDs.push_back(iTrig+iConv+iCC+"POGM_ISOB0.15_ISOEC0.15_DXYB1EC1");
	  OptElectronsIDs.push_back(iTrig+iConv+iCC+"POGTNoIso_ISOB0.15_ISOEC0.15_DXYB1EC1");
	  OptElectronsIDs.push_back(iTrig+iConv+iCC+"POGMNoIso_ISOB0.15_ISOEC0.15_DXYB1EC1");
	}
      }
    }
  ...
  if (MuonOpt){
    ...
    for(auto iMVAB : vMVAB) OptMuonsIDs.push_back(iMVAB+"MVAEC-0.7_ISOB0.15_ISOEC0.15_DXYB1EC1");
    for(auto iMVAEC : vMVAEC) OptMuonsIDs.push_back(iMVAEC+"MVAB-0.4_ISOB0.15_ISOEC0.15_DXYB1EC1");
    OptMuonsIDs.push_back("POGT_ISOB0.15_ISOEC0.15_DXYB1EC1");
    OptMuonsIDs.push_back("POGM_ISOB0.15_ISOEC0.15_DXYB1EC1");
...
void HNL_FakeRateOpt::executeEventFromParameter(AnalyzerParameter param, TString El_ID, HNL_LeptonCore::Channel channel){
...
double HNL_FakeRateOpt::ApplyNvtxReweight(int NPV, TString Key){
...
void HNL_FakeRateOpt::MakeDiLepPlots(HNL_LeptonCore::Channel channel, AnalyzerParameter param, Event ev, std::vector<Lepton *> leps,std::vector<bool> blepsT,  TString label, float event_weight){
...
void HNL_FakeRateOpt::MakeNVertexDistPrescaledTrig(HNL_LeptonCore::Channel channel, AnalyzerParameter param, Event ev, std::vector<Lepton *> leps,std::vector<bool> blepsT,  TString label, float event_weight){
...
void HNL_FakeRateOpt::GetFakeRateAndPromptRates(AnalyzerParameter param, std::vector<Lepton *> leps,std::vector<bool> blepsT, std::vector<Jet>    jetCollTight, TString label, float event_weight, float isocut){
...
void HNL_FakeRateOpt::MakePromptRatePlots(TString label, TString tag,AnalyzerParameter param,  std::vector<Lepton *> leps , std::vector<bool> blepsT , std::vector<Jet> jets,  float event_weight, float isocut, Particle MET){
...
void HNL_FakeRateOpt::GetFakeRates(std::vector<Lepton *> leps,std::vector<bool> blepsT,  AnalyzerParameter param,TString tightlabel,  std::vector<Jet> jets,  TString tag,float event_weight, float isocut){
...
void HNL_FakeRateOpt::FillRegionPlots( TString plot_dir, TString region,  std::vector<Jet> jets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met,  double w){
