...
bool  HNL_LeptonCore::RunSignalRegionAK8(HNL_LeptonCore::Channel channel, HNL_LeptonCore::ChargeType analysis_charge ,std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet>  fatjets, TString bjet_wp, Event ev, AnalyzerParameter param, TString channel_string ,  float w){
  ...
  if(GetMass(signal_region1,jets, fatjets) < UpperMassSR1WmassCut  &&GetMass(signal_region1,jets, fatjets) > LowerMassSR1WmassCut){
    ...
    if(PassHMMet&&PassBJetMVeto) Fill_RegionPlots(channel,true,signal_region1 + "_Highmass", param.Name, jets,  fatjets,  electrons, muons,  METv, nPV, w);
    if(PassHMMet&&PassBJetMVeto) PrintEvent(param, "SR1",w);    
    if(PassHMMet&&PassBJetMVeto) Fill_All_SignalRegion1(channel, signal_region1, IsDATA, QToString(analysis_charge), param.Name, jets, fatjets,  electrons, muons,  METv, nPV  ,w,true);
...
bool  HNL_LeptonCore::RunSignalRegionWW(HNL_LeptonCore::Channel channel, HNL_LeptonCore::ChargeType analysis_charge, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet>  fatjets, TString bjet_wp, Event ev, AnalyzerParameter param, TString channel_string ,  float w){
...
bool  HNL_LeptonCore::RunSignalRegionAK4(HNL_LeptonCore::Channel channel, HNL_LeptonCore::ChargeType analysis_charge, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet>  fatjets, TString bjet_wp, Event ev, AnalyzerParameter param, TString channel_string ,  float w){
...
bool HNL_LeptonCore::RunSignalRegionTrilepton(HNL_LeptonCore::Channel channel, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet>  fatjets, TString bjet_wp, Event ev, AnalyzerParameter param, TString channel_string ,  float w){
...
void HNL_LeptonCore::Fill_RegionPlots(HNL_LeptonCore::Channel channel, bool plotCR, TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<Jet> jets_vbf,   std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w){
  ...
  if (PassVBF(jets_vbf, leps))       FillHist( label_2+"/RegionPlots_"+ label_1+"/"+"PassVBF",  1 ,w, 2,  0., 2,"PassVBF");
  else FillHist( label_2+"/RegionPlots_"+ label_1+"/"+"PassVBF",  0 ,w, 2,  0., 2,"PassVBF");
...
bool HNL_LeptonCore::PassVBFInitial(vector<Jet>  jets){
