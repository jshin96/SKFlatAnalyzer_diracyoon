#include "HNL_FakeRateOpt.h"

void HNL_FakeRateOpt::initializeAnalyzer(){
...
void HNL_FakeRateOpt::executeEvent(){
  ...
  ELIDs.push_back(make_pair("HN"+era,          "HNVeto2016"));   el_loose_id.push_back("HNLoose"+era); 
  // passIDHN(3,0.05, 0.05, 0.1,0.1, 4.,4., 0.1, 0.05, -999., -999.)&&PassMVA(0.65,0.81, 0.89)
  ELIDs.push_back(make_pair("HNRelaxedIP"+era, "HNVeto2016"));   el_loose_id.push_back("HNRelaxedIPLoose"+era);
  ...
  ELIDs.push_back(make_pair("passHEEPID",   "HNVeto2016")) ;  el_loose_id.push_back("HEEPLoose");
  ...
  ELIDs.push_back(make_pair("SUSYTight",   "HNVeto2016")) ;  el_loose_id.push_back("SUSYLoose");
  

  
  paramnames.push_back("HNTight_17028"  );
  paramnames.push_back("HN"+era         );
  paramnames.push_back("HNRelaxedIP"+era);
  paramnames.push_back("HNTightV2"      );
  paramnames.push_back("passPOGTight"  );
  paramnames.push_back("passPOGMedium"  );
  paramnames.push_back("HEEP"  );
  paramnames.push_back("MVAID_noIso_WP80"  );
  paramnames.push_back("MVAID_noIso_WP90"  );
  paramnames.push_back("MVAID_Iso_WP80"  );
  paramnames.push_back("MVAID_Iso_WP90"  );
  paramnames.push_back("SUSY"  );
  
  ELIDs.push_back(make_pair("HN"+era, "HNVeto2016"));           el_loose_id.push_back("HNLoose"+era+"_LooseIP");
  ELIDs.push_back(make_pair("HNRelaxedIP"+era, "HNVeto2016"));  el_loose_id.push_back("HNLoose"+era+"_LooseIP");
  ELIDs.push_back(make_pair("HNTightV2", "HNVeto2016"));        el_loose_id.push_back("HNLooseV4_LooseIP");
  ELIDs.push_back(make_pair("passPOGMedium","HNVeto2016")) ;    el_loose_id.push_back("HNLoosePOG_LooseIP");
  
  paramnames.push_back("HN"+era+"_LIP"    );
  paramnames.push_back("HNRelaxedIP"+era+"_LIP");
  paramnames.push_back("HNTightV2_LIP");
  paramnames.push_back("passPOGMedium_LIP");
  ...
  MuIDs.push_back(make_pair("HNTightV1","HNVeto2016"));       mu_loose_id.push_back("HNLooseV1");
  MuIDs.push_back(make_pair("HNLooseMVA","HNVeto2016"));       mu_loose_id.push_back("HNLooseMVA");
  paramnames.push_back("POGTightWithTightIso");
  paramnames.push_back("HNTight_17028");
  paramnames.push_back("HNTightV2");
  paramnames.push_back("HNTightV1");
...
void HNL_FakeRateOpt::executeEventFromParameter(AnalyzerParameter param, TString El_ID, TString channel){
  ...
  if(!EE) {
    if(param.Name.Contains("MVA")){
      ...
      for(unsigned int j1=0; j1 < 46; j1++){
	double mva_1 = 0.+0.02*double(j1);          std::string mva1= std::to_string(mva_1);
...
void HNL_FakeRateOpt::RunM(std::vector<Electron> loose_el,  std::vector<Muon> loose_mu, std::vector<Jet> jets,   AnalyzerParameter param,  float event_weight){
...
void HNL_FakeRateOpt::RunE( std::vector<Electron> loose_el, std::vector<Muon> loose_mu, std::vector<Jet> jets, AnalyzerParameter param,  float event_weight){
...
void HNL_FakeRateOpt::GetFakeRateAndPromptRates(std::vector<Lepton *> leps,std::vector<bool> blepsT,AnalyzerParameter param, TString label, float event_weight, float isocut){
  ...
  std::vector<Jet>      jetColl   = GetJets(param, "HNTight", 30., 2.5);
...
void HNL_FakeRateOpt::MakePromptRatePlots(TString label, TString tag,AnalyzerParameter param,  std::vector<Lepton *> leps , std::vector<bool> blepsT , std::vector<Jet> jets, std::vector<Jet> alljets, float event_weight, float isocut, Particle MET){
...
float HNL_FakeRateOpt::GetPrescale(std::vector<Lepton *>   leps  ){
...
void HNL_FakeRateOpt::MakeFakeRatePlots(TString label, TString mutag,AnalyzerParameter param,  std::vector<Lepton *> leps , std::vector<bool> blepsT ,  std::vector<Jet> jets, std::vector<Jet> alljets, float event_weight, float isocut, Particle MET){
...
bool HNL_FakeRateOpt::UseEvent(std::vector<Lepton *> leps ,  std::vector< Jet> jets, float awayjetcut, Particle MET, float wt){
...
void HNL_FakeRateOpt::GetFakeRates(std::vector<Lepton *> leps,std::vector<bool> blepsT,  AnalyzerParameter param,TString tightlabel,  std::vector<Jet> jets,  std::vector<Jet> lljets, TString tag,float event_weight, float isocut){
...
HNL_FakeRateOpt::HNL_FakeRateOpt(){
...
TString HNL_FakeRateOpt::SingleElectronPD(TString hlt){
...
TString HNL_FakeRateOpt::SingleMuonPD(TString hlt){
...
void HNL_FakeRateOpt::FillRegionPlots( TString label_1,  std::vector<Jet> jets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met,  double w){
...
void HNL_FakeRateOpt::FillEventCutflow(int charge_i, int cf,float wt, TString cut,   TString label){
...
void HNL_FakeRateOpt::FillWeightHist(TString label, double _weight){
  FillHist( "weights/"+ label , _weight ,1., 200, 0., 5,"ev weight");
}
void HNL_FakeRateOpt::FillCutFlow(bool IsCentral, TString suffix, TString histname, double weight){
