...
void HNL_LeptonFakeRate::executeEvent(){
  ...
  ELIDs.push_back(make_pair("HNL_ULID_"+era, "HNL_FO_ULID_"+era));
  ...
  MuIDs.push_back(make_pair("HNL_TopMVA_MM", "HNL_FO_TopMVA_MM"));
  paramnames.push_back("HNL_TopMVA_MM"  );
...
void HNL_LeptonFakeRate::GetFakeRates(std::vector<Lepton *> leps,std::vector<bool> blepsT,  AnalyzerParameter param,TString tightlabel,  std::vector<Jet> jets,  TString tag,float event_weight, float isocut){
  ...
  for(int ilep = 0 ; ilep < 2; ilep++)  {
    ...
    if(fill_plot) {
      FillHist((prefix + "_pt_eta").Data(), lep_pt_corr, lep_eta,  weight_ptcorr, nbin_pt, ptbins, nbin_eta , etabins);
    }
  }
  return;
}
