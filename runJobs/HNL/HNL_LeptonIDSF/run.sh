...
if [[ $1 == "" ]]; then
    ...
    for i in "${era_list[@]}"
    do
	#SKFlat.py -a $analyzer  -l $datapath/${i}_SingleLepton_E.txt  -n 200  --nmax 100   -e ${i}  --skim SkimTree_DileptonBDT&
	SKFlat.py -a $analyzer  -l   $mcpath/MC.txt  -n 200  --nmax 50  -e ${i}  --skim SkimTree_DileptonBDT&
    done
fi
