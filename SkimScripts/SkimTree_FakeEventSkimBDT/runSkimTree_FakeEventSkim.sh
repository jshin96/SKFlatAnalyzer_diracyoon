...
if [[ $1 == "" ]]; then
    for i in "${era_list[@]}"
    do
        #SKFlat.py -a $analyzer  -i  DYJetsToTauTau_MiNNLO     -n 400  --nmax ${nmax}   -e ${i}  --skim SkimTree_FakeEventSkim&
        SKFlat.py -a $analyzer  -i  DYJetsToEE_MiNNLO         -n 400  --nmax ${nmax}   -e ${i}  --skim SkimTree_FakeEventSkim&
        SKFlat.py -a $analyzer  -i  DYJetsToMuMu_MiNNLO       -n 400  --nmax ${nmax}   -e ${i}  --skim SkimTree_FakeEventSkim&
