...
MCCorrection::MCCorrection() : IgnoreNoHist(false)
{
...
double MCCorrection::MuonReco_SF(TString key, double eta, double p, int sys)
{
  ...
  if (!this_hist)
  {
    ...
    {
      cerr << "[MCCorrection::MuonReco_SF] No "
           << "RECO_SF_" + key << endl;
      exit(ENODATA);
    }
  }
...
double MCCorrection::MuonID_SF(TString ID, double eta, double pt, int sys)
{
  ...
  if (!this_hist)
  {
    ...
    {
      cerr << "[MCCorrection::MuonID_SF] No "
           << "ID_SF_" + ID << endl;
      exit(ENODATA);
    }
  }
...
double MCCorrection::MuonISO_SF(TString ID, double eta, double pt, int sys)
{
  ...
  if (!this_hist)
  {
    ...
    {
      cerr << "[MCCorrection::MuonISO_SF] No "
           << "ISO_SF_" + ID << endl;
      exit(ENODATA);
    }
  }
...
double MCCorrection::ElectronID_SF(TString ID, double sceta, double pt, int sys)
{
  ...
  if (ID.Contains("HEEP"))
  {
    ...
    if (DataYear == 2016)
    {
      ...
      if (IsBarrel)
        this_SF_systerr = (pt < 90. ? 0.01 : min(1. + (pt - 90.) * 0.0022, 3.) * 0.01) * this_SF;
      else
        this_SF_systerr = (pt < 90. ? 0.01 : min(1. + (pt - 90.) * 0.0143, 4.) * 0.01) * this_SF;
    ...
    else if (DataYear == 2017)
    {
      ...
      if (IsBarrel)
        ...
        this_SF_systerr = (pt < 90. ? 0.02 : min(1. + (pt - 90.) * 0.0143, 5.) * 0.01) * this_SF;
  ...
  else
  {
    ...
    if (!this_hist)
    {
      ...
      {
        cerr << "[MCCorrection::ElectronID_SF] (Hist) No "
             << "ID_SF_" + ID << endl;
        exit(ENODATA);
      }
    }
...
double MCCorrection::ElectronReco_SF(double sceta, double pt, int sys)
{
  ...
  if (!this_hist)
  {
    ...
    {
      cerr << "[MCCorrection::ElectronReco_SF] No "
           << "RECO_SF_" + ptrange << endl;
      exit(ENODATA);
    }
  }
...
double MCCorrection::GetPileUpWeight(int N_pileup, int syst)
{
  ...
  int this_bin = N_pileup + 1;
  if (this_bin > this_hist->GetNbinsX())
    this_bin = this_hist->GetNbinsX();
  ...
  for (unsigned int i = 0; i < jetTaggingPars.size(); i++)
  {
    ...
    while (getline(in, btagline))
    {
      ...
      if (tmp_tagger != this_tagger)
        continue;
      if (tmp_pd != "All")
        continue; // don't no period dep now
      ...
      //==== Now, contructing BTagCalibrationReader object
      ...
      if (tmp_tagger.compare("DeepJet_C") == 0)
      {
        systvec_L = {"up", "up_correlated", "up_uncorrelated", "down", "down_correlated", "down_uncorrelated"};
        systvec_C = {"up", "down"};
        systvec_B = {"up", "down"};
      }
...
//  TString mcjetpath = datapath + "/" + DataEra + "/BTag/MeasureJetTaggingEfficiency_TTLJ_powheg.root";
  ...
  for (const auto &obj : *(fmcjet.GetListOfKeys()))
  {
    ...
    if (!hnum.Contains("num"))
      continue;
    TString hden = "";
    if (hnum.Contains("_B_num"))
      hden = "Jet_" + DataEra + "_eff_B_denom";
    else if (hnum.Contains("_C_num"))
      hden = "Jet_" + DataEra + "_eff_C_denom";
    else if (hnum.Contains("_Light_num"))
      hden = "Jet_" + DataEra + "_eff_Light_denom";
...
double MCCorrection::GetBTaggingReweight_1a(const vector<Jet> &jets, JetTagging::Parameters jtp, string Syst)
{
  ...
  bool Syst_HTag = false, Syst_LTag = false;
  int SystDir = 0, CorrType = 0;
  string SystKey;
  if (SystStr.Contains("Syst"))
  {
    if (SystStr.Contains("HTag"))
      Syst_HTag = true;
    else if (SystStr.Contains("LTag"))
      Syst_LTag = true;
    if (SystStr.Contains("Up"))
      SystDir = 1;
    else if (SystStr.Contains("Down"))
      SystDir = -1;
    if (SystStr.Contains("UnCorr"))
      CorrType = -1;
    else if (SystStr.Contains("Corr"))
      CorrType = 1;
    if (SystDir == 0)
    {
      cout << "SystStr in not correct form" << endl;
      exit(ENODATA);
    }
    if (!(Syst_HTag or Syst_LTag))
    {
      cout << "SystMode but no H/L mode assigned" << endl;
      exit(ENODATA);
    }
  }
  ...
  for (unsigned int i = 0; i < jets.size(); i++)
  {
    ...
    if (Syst_HTag && (JetHadFlav == 4 or JetHadFlav == 5))
    {
      ApplySyst = true;
    }
    else if (Syst_LTag && (JetHadFlav == 0))
    {
      ApplySyst = true;
    }

    if (ApplySyst && CorrType == 0)
      SystKey = SystDir > 0 ? "up" : "down";
    else if (ApplySyst && CorrType > 0)
      SystKey = SystDir > 0 ? "up_correlated" : "down_correlated";
    else if (ApplySyst && CorrType < 0)
      SystKey = SystDir > 0 ? "up_uncorrelated" : "down_uncorrelated";
    else
      SystKey = "central";
...
double MCCorrection::GetBTaggingReweight_1d(const vector<Jet> &jets, JetTagging::Parameters jtp, string Syst)
{
  ...
  for (unsigned int i = 0; i < jets.size(); i++)
  {
    ...
    */
    ...
    if (abs_hadFlavour == 5)
    {
      if (tmp_Syst.Contains(TRegexp("hfstats[1-2]$")))
        GoodSyst = true;
      else if (tmp_Syst.Contains(TRegexp("lf$")))
        GoodSyst = true;
      else if (tmp_Syst.Contains(TRegexp("jes$")))
        GoodSyst = true;
    }
    else if (abs_hadFlavour == 4)
    {
      if (tmp_Syst.Contains(TRegexp("cferr[1-2]$")))
        GoodSyst = true;
    }
    else
    {
      if (tmp_Syst.Contains(TRegexp("hf$")))
        GoodSyst = true;
      else if (tmp_Syst.Contains(TRegexp("jes$")))
        GoodSyst = true;
      else if (tmp_Syst.Contains(TRegexp("lfstats[1-2]$")))
        GoodSyst = true;
    }
