...
AnalysisCode="GetEffLumi" #"BTagEff_MC" 
Skim=""
RunningMode=""
...
#                 - Opt(Mult): runData/runBkdMC/runSigMC
...
do
  ...
  do
    ...
    elif [[ ${runMode} == "runData" ]]; then List="${AnalysisCode}_${CategoryLabel}_${Year}"; 
    else :; fi
    ...
    do
      ...
      DataPeriod="ALL"
      if [[ ${runMode} == "runData" ]]; then DataPeriod=$( python GetJobConfig.py ${List} ${it_s} | cut -d ' ' -f3 ); fi
      ...
      if [[ ${runDebug} == "True" ]]; then 
        ...
        PeriodLabel=""
        if [[ ${runMode} == "runData" ]]; then PeriodLabel="_period${DataPeriod}"; fi
