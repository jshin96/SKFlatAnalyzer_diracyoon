void runIDBDT_HNtypeIFake(TString channel="EE",  int signal_mode=2, TString NTrees="1000", TString NormMode="NumEvents", TString   MinNodeSize ="1.5", TString MaxDepth = "3", TString nCuts="100", bool IgnoreNegweights=true){
  ...
  //HNL_LepIDKinVar_DYTypeI_DF_M100_private.root
  
  TFile* fsin = TFile::Open(path+"HNL_LepIDKinVar_"+signal+".root");
  TFile* fbin = TFile::Open(path+"HNL_LepIDKinVar_FakeBkg.root");

  cout << path+"HNL_LepIDKinVar_Signal"+signal+".root" << endl;
  ...
  //data_loader->AddVariable("HNTightID", "HNTightID", "units", 'I');
  ...
  //==== Gradient Boost
  //factory->BookMethod(data_loader, TMVA::Types::kBDT, "BDTG",  "!H:!V:NTrees="+NTrees+":MinNodeSize="+MinNodeSize+"%:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.6:nCuts="+nCuts+":MaxDepth="+MaxDepth );
