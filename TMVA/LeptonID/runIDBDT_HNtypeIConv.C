void runIDBDT_HNtypeIConv(TString Classifier ="BDTG" ,TString BkgType = "Conv", TString era="2017",TString channel="EE",  int signal_mode=1, int var_mode=0, TString NTrees="800", TString NormMode="NumEvents", TString   MinNodeSize ="1.5", TString MaxDepth = "4", TString nCuts="300", TString BoostLearningRate="0.05", TString BaggedFrac="0.6", TString seed = "100",  int eta_mode= 0){
  ...
  TString SigTag = "SignalConv";
  if(var_mode == 1) SigTag = "SignalConv_MVA";
  if(var_mode == 2) SigTag = "SignalConv_IP";

  TString signal="";

  if(signal_mode == 1) signal=SigTag+"_BB"; // Pt
  else if(signal_mode == 2) signal=SigTag+"_EC";
  else return;

  if(signal_mode == 3) signal=SigTag+"_LPtBB";
  if(signal_mode == 4) signal=SigTag+"_LPtEC";
  ...
  //  if(signal_mode==3 || signal_mode==4 )  signame = path+"HNL_LeptonID_BDT_KinVar_LowPtPrompt.root";
  //  if(signal_mode==5 || signal_mode==6 )  signame = path+"HNL_LeptonID_BDT_KinVar_HNLPrompt.root";
  ...
  //  data_loader->AddVariable("POGMVA80","POGMVA80", "units", 'F');
